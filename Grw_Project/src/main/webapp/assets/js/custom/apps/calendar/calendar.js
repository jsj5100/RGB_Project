...
var KTAppCalendar = function() {
    ...
    const M = () => {
        v.innerText = "Add a New Event";
        u.show();

        const o = f.querySelectorAll('[data-kt-calendar="datepicker"]');
        const i = f.querySelector("#kt_calendar_datepicker_allday");

        i.addEventListener("click", (e => {
            e.target.checked
                ? o.forEach(e => { e.classList.add("d-none"); })
                : (l.setDate(E.startDate, !0, "Y-m-d"), o.forEach(e => { e.classList.remove("d-none"); }));
        }));
                        ...
                        setTimeout(() => {
                            ...
                            Swal.fire({
                                ...
                                if (o.isConfirmed) {
                                    ...
                                    let o = !1;
                                    if (i.checked) o = !0;
                                    if (0 === c.selectedDates.length) o = !0;

                                    var d = moment(r.selectedDates[0]).format();
                                    var s = moment(l.selectedDates[l.selectedDates.length - 1]).format();

                                    if (!o) {
                                        const e = moment(r.selectedDates[0]).format("YYYY-MM-DD");
                                        d = e + "T" + moment(c.selectedDates[0]).format("HH:mm:ss");
                                        s = e + "T" + moment(m.selectedDates[0]).format("HH:mm:ss");
                                    }
    ...
    const B = () => {
        ...
        E.allDay
            ? (e = "All Day", t = moment(E.startDate).format("Do MMM, YYYY"), n = moment(E.endDate).format("Do MMM, YYYY"))
    ...
    const C = () => {
        ...
        const i = f.querySelectorAll('[data-kt-calendar="datepicker"]');

        E.allDay
            ? (o.checked = !0, i.forEach(e => { e.classList.add("d-none"); }))
            : (c.setDate(E.startDate, !0, "Y-m-d H:i"), m.setDate(E.endDate, !0, "Y-m-d H:i"), l.setDate(E.startDate, !0, "Y-m-d"), o.checked = !1, i.forEach(e => { e.classList.remove("d-none"); }));
    };
    ...
    return {
        init: function() {
            ...
            const F = document.getElementById("kt_calendar_app");
            const O = moment().startOf("day");
            const I = O.format("YYYY-MM");
            const R = O.clone().subtract(1, "day").format("YYYY-MM-DD");
            const V = O.format("YYYY-MM-DD");
            const P = O.clone().add(1, "day").format("YYYY-MM-DD");
