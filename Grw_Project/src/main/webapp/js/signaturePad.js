document.addEventListener('DOMContentLoaded', () => {
	...
	//서명 수정 모달이 열릴 때 캔버스 크기 조정
	const modifyModal = document.getElementById('kt_modal_modify');
	...
	document.getElementById('saveSign').addEventListener('click', async () => {
		if (signaturePad.isEmpty()) {
			...
			try {
				const response = await fetch('http://localhost:8080/Grw_Project/saveSignatureSign.do', {
					method: 'POST',
					headers: {
